...
void ElectronIdSelector::produce(edm::Event& iEvent,const edm::EventSetup& iSetup)
{
  ...
  for(unsigned int iElec=0; iElec<electrons->size(); iElec++) { 
    ...
    // ---------- cut-based ID -----------------
    isTight = (pt>20.) && (mHits<=0) &&
        (isolation<0.10) && (!vtxFitConversion) &&
        ((isEB && sigmaIEtaIEta<0.01 && dPhiIn<0.03 && dEtaIn<0.004 && hoe<0.120 && ooemoop<0.050 && fabs(d0vtx)<0.020 && fabs(dzvtx)<0.100) ||
         (isEE && sigmaIEtaIEta<0.03 && dPhiIn<0.02 && dEtaIn<0.005 && hoe<0.100 && ooemoop<0.050 && fabs(d0vtx)<0.020 && fabs(dzvtx)<0.100));
...
// plugin definition
////////////////////////////////////////////////////////////////////////////////
typedef ElectronIdSelector         		InvHiggsPATElectronIdSelector;
